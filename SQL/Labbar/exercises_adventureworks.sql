...
/* 
The code joins object_Id from the schema 'sys' on columns and tables.
It then looks at where the name has anything with 'BusinessEntity' in it.
Finally it orders the results.
*/
...
You want to sort the data so the highest TotalDue comes first. 
What is the highest order amount and when was it shipped?
...
and Sales.SalesPerson (as B). Order the table so that those who had highest
...
SELECT TOP 10
	...
	B.SalesYTD, 
	B.SalesLastYear 
FROM Person.Person as A JOIN Sales.SalesPerson as B
	on A.BusinessEntityID = B.BusinessEntityID
ORDER BY B.SalesLastYear desc;

-- 3 -- "less than 5,000,000" är alla??!
SELECT 
	A.BusinessEntityID, 
	A.FirstName, 
	A.LastName, 
...
FROM Person.Person as A JOIN Sales.SalesPerson as B
	on A.BusinessEntityID = B.BusinessEntityID
WHERE (SalesLastYear <= 5000000)
ORDER BY B.SalesLastYear asc;
...
/*
Markera Databasen och refresha för att se den. 
Verkar inte fungera att bara trycka f5 i explorern.
*/
...
/*
En View är inte redigerbar av användaren så att DB ändras. 
Det är bara en query som återanvänds.
*/
